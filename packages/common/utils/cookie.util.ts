export const docCookies = {
  ...
  removeRecursively: function (sKey: string, sPath?: string, sDomain?: string) {
    const _sPath = sPath ?? location.pathname;
    const _sDomain = sDomain ?? location.hostname;

    _sPath.split(/\//).reduce((accurate, path) => {
      const current = accurate + (path ? path + '/' : path);
      this.removeItem(sKey, current, _sDomain);
      return current;
    }, '/');
  },
